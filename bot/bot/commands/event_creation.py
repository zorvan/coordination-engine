...
def build_location_type_markup(prefix: str = "event") -> InlineKeyboardMarkup:
    ...
    options = [
        (label, f"{prefix}_location_{value}")
...
def build_transport_markup(prefix: str = "event") -> InlineKeyboardMarkup:
    ...
    options = [
        (label, f"{prefix}_transport_{value}")
...
def build_calendar_markup(year: int, month: int, prefix: str = "event") -> InlineKeyboardMarkup:
    ...
    rows.append(
        [InlineKeyboardButton("✏️ Edit Previous", callback_data=f"{prefix}_edit_date_preset")]
    )
...
async def _apply_final_stage_patch(
    ...
    if not is_private and str(flow_data.get("scheduling_mode", "fixed")) == "fixed" and not flow_data.get("scheduled_time"):
...
async def _handle_callback_common(
    ...
    if data and data.startswith(f"{prefix}_edit_"):
        ...
        if target == "description":
            ...
            event_flow["stage"] = "threshold"
            context.user_data[flow_key] = event_flow
            back_target = f"{prefix}_edit_time_window" if not (mode == "public" and scheduling_mode == "flexible") else f"{prefix}_edit_type"
            await query.edit_message_text(
    ...
    if stage == "description":
        ...
        try:
            ...
            await update.message.reply_text(
                f"⏱️ *Time: {scheduled_time.strftime('%Y-%m-%d %H:%M')}*\n\n"
                "What is the minimum attendance threshold?",
                reply_markup=build_threshold_markup("event_edit_time_window" if mode == "public" else "private_event_edit_time_window"),
            )
        except ValueError:
            await update.message.reply_text(
...
async def finalize_event(query: CallbackQuery, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await context.bot.send_message(
        ...
        text=(
            ...
            f"Event ID: {event.event_id}\n"
            f"State: locked\n\n"
            "Event has been locked. Only you (the event admin) can manage it.\n"
            "All event coordination happens in private DM."
        ),
