...
ensure_required_env() {
  ...
  case "${TELEGRAM_TOKEN}" in
    test-token|dummy-token|changeme|your-token-here|"<set-me>")
      ...
      echo "Set a real BotFather token in .env before running resume.sh."
      exit 1
      ;;
  esac
...
ensure_local_db_running() {
  ...
  if docker ps -a --format '{{.Names}}' | grep -qx "$DB_CONTAINER_NAME"; then
    ...
    echo "Creating persistent local PostgreSQL container: $DB_CONTAINER_NAME"
    docker run -d \
...
if is_local_db_url; then
  ...
  echo "DB_URL points to a non-local database. Skipping Docker management."
fi
