...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async for session in get_session(settings.db_url):
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("Manage Constraints", callback_data=f"event_constraints_{event_id}")],
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("event_logs_"):
        event_id = int(data.replace("event_logs_", ""))
        await show_logs(query, event_id)
    elif data.startswith("event_constraints_"):
...
async def show_logs(query, event_id: int) -> None:
    ...
    async for session in get_session(settings.db_url):
        ...
        result = await session.execute(
            Log.__table__.select().where(Log.event_id == event_id).order_by(Log.timestamp.desc())
        )
...
async def show_constraints(query, event_id: int) -> None:
    ...
    async for session in get_session(settings.db_url):
        ...
        for c in constraints:
            ...
            if c.target_user_id:
                msg += f"Join if User {c.target_user_id} joins (confidence: {c.confidence})\n"
            else:
                msg += f"{c.type}\n"
